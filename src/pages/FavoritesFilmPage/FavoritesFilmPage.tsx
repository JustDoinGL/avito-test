...
const FavoritesFilmPage = () => {
  ...
  const { favoriteFilms, favoriteFilmsState } = useAppSelector((store) => store.films)

  const filmsPerPage = 5

  useEffect(() => {
    const loadFilms = async () => {
      const filmsToLoad = favoriteFilms.slice(currentPage * filmsPerPage, (currentPage + 1) * filmsPerPage)
  ...
  return (
    <div className='container' data-testid='favorites-page'>
      ...
      <ButtonTop />
      {favoriteFilms.length === 0 && <h2 className='h2'>Избранные фильмы не выбраны</h2>}
