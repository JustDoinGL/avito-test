...
const ReviewCard = ({ comment }: ReviewCardProps) => {
  ...
  const summaryText = comment.review.length > 100 ? `${comment.review.slice(0, 100)}...` : comment.review
  ...
  return (
    <div className={styles.card}>
      ...
      <div className={styles.cardFooter}>
        ...
        <h5 className={styles.like}>Нравится: {comment.reviewLikes ? comment.reviewLikes : 0}</h5>
        <h5 className={styles.dislike}>Не нравится: {comment.reviewDislikes ? comment.reviewDislikes : 0}</h5>
      </div>
    </div>
  )
}
